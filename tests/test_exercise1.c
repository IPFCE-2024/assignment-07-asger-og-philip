...
int main() {
    ...
    double result3 = taylor_sine(M_PI / 4.0, 5);
    assert(fabs(result3 - sin(M_PI / 4.0)) < EPSILON && "Test 3 failed: sin(π/4) should be ≈ 0.7071");
    printf("Test 3 passed: sin(π/4) ≈ 0.7071\n");
    
    // Test 4: sin(π/2) = 1 (90 degrees)
    double result4 = taylor_sine(M_PI / 2.0, 5);
    assert(fabs(result4 - 1.0) < EPSILON && "Test 4 failed: sin(π/2) should be ≈ 1.0");
    printf("Test 4 passed: sin(π/2) = 1\n");
    
    // Test 5: sin(π) ≈ 0 (180 degrees)
    double result5 = taylor_sine(M_PI, 5);
    assert(fabs(result5 - 0.0) < EPSILON && "Test 5 failed: sin(π) should be ≈ 0");
    printf("Test 5 passed: sin(π) ≈ 0\n");
    
    // Test 6: sin(-π/6) ≈ -0.5 (negative angle)
    double result6 = taylor_sine(-M_PI / 6.0, 5);
    ...
    double result7 = taylor_sine(1.0, 5);
    assert(fabs(result7 - sin(1.0)) < EPSILON && "Test 7 failed: sin(1.0) should match math.h sin");
